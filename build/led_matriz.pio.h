...
#define led_matriz_wrap_target 0
#define led_matriz_wrap 6
#define led_matriz_pio_version 0

static const uint16_t led_matriz_program_instructions[] = {
            //     .wrap_target
    0x6021, //  0: out    x, 1                       
    0x0024, //  1: jmp    !x, 4                      
...
#if !PICO_NO_HARDWARE
static const struct pio_program led_matriz_program = {
    .instructions = led_matriz_program_instructions,
    .length = 7,
    .origin = -1,
    .pio_version = led_matriz_pio_version,
#if PICO_PIO_VERSION > 0
    .used_gpio_ranges = 0x0
#endif
};

static inline pio_sm_config led_matriz_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + led_matriz_wrap_target, offset + led_matriz_wrap);
    return c;
}

static inline void led_matriz_program_init(PIO pio, uint sm, uint offset, uint pin)
{
    pio_sm_config c = led_matriz_program_get_default_config(offset);
